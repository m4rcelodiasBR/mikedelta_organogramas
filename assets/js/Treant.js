    ...
    firstWalk: function (node, level) {
      ...
      if (node.childrenCount() === 0 || level == this.CONFIG.maxDepth) {
        ...
        if (node.stackParent) {
           ...
           * ==================================================
           */
          var offset = node.size() / 2; // Padrão
          if (node.meta && node.meta.posicao && node.meta.posicao !== 3) {
            var pos = node.meta.posicao;
            var mult = pos > 3 ? pos - 1 : pos; // Se for 4 ou 5, subtrai 1 para a matemática fechar
            var partWidth = node.size() / 4;
            offset = partWidth * mult - partWidth / 2;
          }
  ...
  TreeNode.prototype = {
    ...
    connectorPoint: function (startPoint) {
      ...
      if (orient === "NORTH") {
        ...
        if (startPoint) {
          if (this.meta && this.meta.posicao && this.meta.posicao !== 3) {
            var pos = this.meta.posicao;
            var mult = pos > 3 ? pos - 1 : pos;
          ...
          if (
            parentNode &&
            parentNode.meta &&
            parentNode.meta.posicao &&
            parentNode.meta.posicao !== 3
          ) {
            var pos = parentNode.meta.posicao;
            var mult = pos > 3 ? pos - 1 : pos;
            var partWidth = this.width / 4;
            offset = partWidth * mult - partWidth / 2;
          }
